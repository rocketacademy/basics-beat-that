...
var main = function (input) {
  ...
  if (gameMode == "enter username") {
    ...
    username = input;
    gameMode = "game start";
    myOutputValue = `Welcome ${username} <br><br> Press Submit to roll Dice. `;
  ...
  if (gameMode == "player 2") {
    ...
    if (dicePlayerTwo < dicePlayerOne) {
      ...
      winTrackerP2 += 1;
      numberOfTries += 1;
      myOutputValue = `Player 2 rolled ${dicePlayerTwo} <br><br> ${username} lost! <br><br> ${username}'s guess of ${dicePlayerOne} is lesser than ${dicePlayerTwo}. <br><br> ${username}: ${winTrackerP1} wins. <br> Player 2: ${winTrackerP2}. <br> Number of tries: ${numberOfTries} <br><br> Press submit to try again!`;
    }
  }
  return myOutputValue;
};
