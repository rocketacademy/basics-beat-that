...
var playerTurn = function (playerNumber, choice, playerArray) {
  if (playerNumber == 1 && choice == null) {
    ...
    if (playerNumber == 1) {
      player1diceResults = `${playerArray[0]}${playerArray[1]}`;
    } else if (playerNumber == 2) {
      player2diceResults = `${playerArray[0]}${playerArray[1]}`;
    }
    ...
    player1diceResults = `${playerArray[0]}${playerArray[1]}`;
    console.log(
      `player ${playerNumber} dice results: ${playerArray[0]}${playerArray[1]}`
    );
    console.log(`user dice order: ${choice}`);
    return myOutputValue;
  } else if (choice == 2) {
    if (playerNumber == 1) {
      player1diceResults = `${playerArray[1]}${playerArray[0]}`;
    } else if (playerNumber == 2) {
      player2diceResults = `${playerArray[1]}${playerArray[0]}`;
    }
    myOutputValue = `${playerNumber} have chose dice 2 to be the first in order, your result is ${playerArray[1]}${playerArray[0]} <br><br>
    Click enter for next.`;

    console.log(
      `player ${playerNumber} dice results: ${playerArray[1]}${playerArray[0]}`
    );
    console.log(`user dice order: ${choice}`);
    return myOutputValue;
  }
...
var gameOutcome = function () {
  if (Number(player1diceResults) > Number(player2diceResults)) {
    myOutputValue = `Congratulations ${player1} for winning!!! <br><br>
    ${player1} BEATS ${player2} with a score of ${Number(
      player1diceResults
    )} against ${Number(player2diceResults)} <br><br>
    Click enter to start another new game`;
    return myOutputValue;
  } else if (Number(player2diceResults) > Number(player1diceResults)) {
    myOutputValue = `Congratulations ${player2} for winning!!! <br><br>
     ${player2} BEATS ${player1} with a score of ${Number(
      player2diceResults
    )} against ${Number(player1diceResults)}<br><br>
    Click enter to start another new game`;
    return myOutputValue;
  } else if (Number(player2diceResults) == Number(player1diceResults)) {
