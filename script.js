...
// ======================= MORE COMFORTABLE (SCORE + LEADERBOARD + LOWEST COMBINED NUMBER MODE + AUTO-GENERATE COMBINED NUMBER + VARIABLE NUMBER OF DICE + VARIABLE NUMBER OF PLAYERS + KNOCKOUT MODE) ===========================
...
var main = function (input) {
  ...
  if (gameMode == GAME) {
    ...
    if (numberOfDice != UNASSIGNED) {
      ...
      // If this is neither the first or last player's turn
      if (playerNo != 0 && playerNo != numberOfPlayers - 1) {
        // Find winner based on running sum thus far
        var winningPlayer = findWinner(competingTotals, resultMode);
        console.log("competingTotals", competingTotals);
        console.log("winningPlayer", winningPlayer);
        ...
        // Output Result message
        return (
          `You rolled these numbers: ${unsortedDiceRollArray}. <br> <br> This is your best combined number from those digits above: ${bestNumber}. <br><br><br> 🏆 THE WINNER WHO PROGESSED TO THE NEXT ROUND IS: PLAYER ${winningPlayer}! 🏆 <br> <br> You are currently in the ${resultMode} game mode. <br> <br>` +
          resultLog +
          `<br><br><br> To continue, please get the next player to roll the dice by clicking the submit button. <br><br><br> ================= <br><br> Tip: <br><br> Enter "reversed" for reversed mode (lowest number wins). <br> Enter "normal" for normal mode (highest number wins).`
        );
      }
      // If this is the final player's turn, output the current player's dice roll outcome, along with win/lose results and stats
      if (playerNo == numberOfPlayers - 1) {
